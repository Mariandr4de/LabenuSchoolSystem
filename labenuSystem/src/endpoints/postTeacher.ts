...
import { createTeacherExpertise } from "../data/createTeacherExpertise";
import { connection } from "../data/dataBase";

export const postTeacher = async (req: Request, res: Response) => {
  try {
    const {  teacher_id, teacher_name, teacher_email, birth_date, class_id, expertise_id } = req.body;

    if (!teacher_id || !teacher_name || !teacher_email || !birth_date || !class_id || !expertise_id) {
      res.statusCode = 400;
    ...
    await createTeacher( teacher_id, teacher_name, teacher_email, birth_date, class_id);
